...
class PeakAnalysisEngine:
    ...
    def analyze_peaks(
        ...
        try:
            ...
            background_info = self._fit_background(
                q_vals, intensities, config.fitting
            )
            bg_subtracted = intensities - background_info.background_points
            
            # Peak detection
            peak_indices = self._detect_peaks(
                q_vals, bg_subtracted, config.detection
            )
