...
class TestSessionSaveLoad:
    ...
    def test_save_and_load_single_file_session(
        ...
        np.testing.assert_allclose(
            loaded_data.q_values,
            sample_diffraction_data.q_values,
            rtol=1e-10
        )
        np.testing.assert_allclose(
            loaded_data.intensities,
    ...
    def test_save_and_load_multiple_files(self, session_manager):
        ...
        # Create session
        session_id = session_manager.create_session(
            name="Multi-file Session",
            description="Session with 3 files"
        )
    ...
    def test_wavelength_preservation(self, session_manager):
        ...
        for filename, wavelength in test_wavelengths.items():
            ...
            session_id = session_manager.create_session(
                name=f"Session {wavelength}",
                description=f"Testing {wavelength} Å"
            )
    ...
    def test_default_wavelength_preservation(self, session_manager):
        ...
        """
        session_id = session_manager.create_session(
            name="Default Wavelength Test",
            description="Testing 0.1665 Å (synchrotron)"
        )
    ...
    def test_simulate_dashboard_save_workflow(self, session_manager):
        ...
        session_id = session_manager.create_session(
            ...
            description="Simulated from dashboard"
        )
    ...
    def test_simulate_dashboard_load_workflow(self, session_manager):
        ...
        session_id = session_manager.create_session(
            ...
            description="For testing load workflow"
        )
        ...
        for session_file in session_files:
            # Load from FileStore
            diffraction = session_manager.file_store.load_file(
