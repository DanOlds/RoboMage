...
from .data_io import load_chi_file, load_test_data, get_data_info
...
def plot_data(df, output_dir=".", filename=None, show=True, save=True):
    ...
    try:
        ...
        print("Error: matplotlib is required for plotting. Install with 'pixi install matplotlib'")
        return False
...
def plot_multiple_data(datasets, labels, output_dir=".", filename=None, show=True, save=True):
    """
    ...
    for i, (df, label, color) in enumerate(zip(datasets, labels, colors)):
        plt.plot(df['Q'], df['intensity'], color=color, linewidth=1, label=label, alpha=0.8)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Input data file to process (.chi format), or 'test' to use built-in test data"
    )
    parser.add_argument(
        "--files", "-f",
        nargs='+',  # One or more files required when used
        help="Process multiple files for comparison. Supports glob patterns like '*.chi'"
    )
    parser.add_argument(
        "--output", "-o",
    ...
    parser.add_argument(
        "--info", "-i",
        action="store_true",
        help="Show data summary information"
    )
    parser.add_argument(
        "--plot", "-p",
        action="store_true",
    ...
    if args.files:
        ...
        print("No input specified. Use a single file argument, 'test', or --files option.")
    ...
    try:
        ...
        if args.plot or args.save_plot:
            ...
            if len(datasets) == 1:
                ...
                df = datasets[0]
                if args.save_plot:
                    success = plot_data(df, output_dir=args.output, filename=args.save_plot, show=False)
                else:
                    success = plot_data(df, output_dir=args.output, filename=None, show=True, save=False)
            else:
                # Multiple files - use multi-dataset plotting
                if args.save_plot:
                    success = plot_multiple_data(datasets, labels, output_dir=args.output, filename=args.save_plot, show=False)
                else:
                    success = plot_multiple_data(datasets, labels, output_dir=args.output, filename=None, show=True, save=False)
