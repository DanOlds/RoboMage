...
class PeakAnalyzerCLI:
    ...
    def start_service(self, port: int = 8001, dev_mode: bool = False) -> bool:
        ...
        try:
            ...
            if client.wait_for_service(max_wait=30.0):
                print(f"✓ Peak analysis service started on http://127.0.0.1:{port}")
    ...
    def analyze_file(
        ...
        try:
            ...
            print(f"✓ Loaded {data.statistics.num_points} data points")
    ...
    def _process_results(
        ...
        print(f"Overall R²: {metadata.get('overall_r_squared', 0):.3f}")
        ...
        if peaks:
            ...
            print("ID | Position (Å⁻¹) | d-spacing (Å) | Height | Width | R²")
            print("-" * 60)
            for peak in peaks:
                print(
                    f"{peak['peak_id']:2d} | "
        ...
        if output_dir:
            ...
            print(f"✓ Peak summary saved to {csv_file}")
    ...
    def _save_peaks_csv(self, peaks: list, csv_file: Path):
        ...
        with open(csv_file, "w", newline="") as f:
            ...
            writer = csv.writer(f)
            writer.writerow([
                "peak_id", "position_A_inv", "d_spacing_A", "height", 
                "width", "area", "r_squared", "profile_type"
            ])
...
Examples:
  ...
  peak-analyzer sample.chi --config analysis_config.json
    ...
    parser.add_argument(
        "files",
        nargs="*",
        help="Diffraction data files to analyze (.chi, .xy, .dat)"
    )
    
    # Output options
    parser.add_argument(
        "--output", "-o",
        help="Output directory for results (JSON, CSV, plots)"
    )
    ...
    parser.add_argument(
        "--config", "-c",
        help="JSON configuration file for analysis parameters"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Minimum distance between peaks (Å⁻¹, default: 0.1)"
    )
