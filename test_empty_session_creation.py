...
def test_empty_session_creation():
    ...
    session_id = mgr.create_session(
        name=session_name,
        description="This session has no files initially"
    )
...
def test_workflow_with_empty_session():
    ...
    session_name = f"Workflow Test Session {datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
    session_id = mgr.create_session(
        name=session_name,
        description="Empty session for workflow testing"
    )
