...
async def test_workflow():
    ...
    workflow = WorkflowDefinition(
        ...
        nodes=[
            ...
            WorkflowNode(
                ...
                type="peak_analysis",
                label="Detect Peaks",
                config={"profile_type": "gaussian", "prominence": 0.1, "distance": 5},
            ...
            WorkflowNode(
                ...
                config={"output_path": "workflow_results.csv", "format": "peaks"},
                position=NodePosition(x=700, y=100),
            ),
        ],
        edges=[
            WorkflowEdge(id="edge_1", source="load_1", target="analyze_1"),
            WorkflowEdge(id="edge_2", source="analyze_1", target="export_1"),
        ],
    )
    ...
    orchestrator.register_node_handler(
        "filter_q_range", data_nodes.filter_q_range_handler
    )
    ...
    orchestrator.register_node_handler(
        "save_results", output_nodes.save_results_handler
    )
    ...
    try:
        ...
        for nr in result.node_results:
            ...
            print(
                f"  {status_icon} {nr.node_id}: {nr.status} ({nr.duration_ms:.1f} ms)"
            )
            if nr.error:
        ...
        print(
            f"Final output keys: {list(result.final_output.keys()) if result.final_output else 'None'}"
        )
