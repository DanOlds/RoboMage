...
def main():
    ...
    for xy_file in available_files:
        ...
        print(
            f"  💡 Intensity range: {stats.intensity_range[0]:.0f} to "
            f"{stats.intensity_range[1]:.0f}"
        )
        print(f"  📊 Mean intensity: {stats.intensity_mean:.1f}")
        uniformity = (1 - stats.q_step_std / stats.q_step_mean) * 100
        print(f"  📐 Q step uniformity: {uniformity:.1f}%")
    ...
    print(
        f"  💡 Intensity range: {df['intensity'].min():.0f} to "
        f"{df['intensity'].max():.0f}"
    )
